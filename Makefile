...
CFLAGS = -Werror -Wall -Wextra -Wpedantic -Wformat=2 -Wconversion -Wshadow -Wcast-qual -Wcast-align -Wstrict-prototypes -Wmissing-prototypes -Wmissing-declarations -Wuninitialized -Warray-bounds -Wvla -Wwrite-strings -Wnull-dereference -Wimplicit-fallthrough -Wsign-conversion -Wsign-compare -Wfloat-equal -Wpointer-arith -Wbad-function-cast -Wstrict-overflow -Waggregate-return -Wredundant-decls -Wnested-externs -Winline -Wswitch-enum -Wswitch-default -Wenum-conversion -Wdisabled-optimization -O2 -std=c11 -D_POSIX_C_SOURCE=200809L -D_DEFAULT_SOURCE=1 -Wno-aggregate-return -fstack-protector-strong -U_FORTIFY_SOURCE -D_FORTIFY_SOURCE=2 -fPIE $(SANITIZERS)
# Add vendor include path if vendor/include exists (for dev environments without system headers)
ifneq ($(wildcard vendor/include),)
    CFLAGS += -Ivendor/include
endif
DEBUG_CFLAGS = -Werror -Wall -Wextra -Wpedantic -Wformat=2 -Wconversion -Wshadow -Wcast-qual -Wcast-align -Wstrict-prototypes -Wmissing-prototypes -Wmissing-declarations -Wuninitialized -Warray-bounds -Wvla -Wwrite-strings -Wnull-dereference -Wimplicit-fallthrough -Wsign-conversion -Wsign-compare -Wfloat-equal -Wpointer-arith -Wbad-function-cast -Wstrict-overflow -Waggregate-return -Wredundant-decls -Wnested-externs -Winline -Wswitch-enum -Wswitch-default -Wenum-conversion -Wdisabled-optimization -Wno-aggregate-return -g -O0 -std=c11 -D_POSIX_C_SOURCE=200809L -D_DEFAULT_SOURCE=1 -fsanitize=address -fno-omit-frame-pointer -fstack-protector-strong -U_FORTIFY_SOURCE -D_FORTIFY_SOURCE=2 -fPIE
...
# Memory system uses SQLite (memory_db.c) - no special build flags needed
# SQLite3 is a required dependency checked in check-deps
...
TEST_SSE_PARSER_TARGET = $(BUILD_DIR)/test_sse_parser
TEST_STREAMING_TOOL_ACCUMULATOR_TARGET = $(BUILD_DIR)/test_streaming_tool_accumulator
TEST_STREAMING_LIVE_TARGET = $(BUILD_DIR)/test_streaming_live
...
MACOS_SQLITE_FIX_SRC = src/macos_sqlite_fix.c
MACOS_SQLITE_FIX_OBJ = $(BUILD_DIR)/macos_sqlite_fix.o
...
SETTINGS_MENU_SRC = src/settings_menu.c
SETTINGS_MENU_OBJ = $(BUILD_DIR)/settings_menu.o
...
LINE_PRINTER_SRC = src/line_printer.c
LINE_PRINTER_OBJ = $(BUILD_DIR)/line_printer.o
MARKDOWN_RENDER_SRC = src/markdown_render.c
MARKDOWN_RENDER_OBJ = $(BUILD_DIR)/markdown_render.o
...
VLTRN_BANNER_SRC = src/vltrn_banner.c
VLTRN_BANNER_OBJ = $(BUILD_DIR)/vltrn_banner.o
...
GOAL_SRC = src/goal.c
GOAL_OBJ = $(BUILD_DIR)/goal.o
$(BUILD_DIR)/goal.o: $(GOAL_SRC) src/goal.h src/klawed_internal.h src/api/api_client.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $@ $<
...
OPENAI_STREAMING_SRC = src/openai_streaming.c
OPENAI_STREAMING_OBJ = $(BUILD_DIR)/openai_streaming.o
OPENAI_CHAT_PARSER_SRC = src/openai_chat_parser.c
OPENAI_CHAT_PARSER_OBJ = $(BUILD_DIR)/openai_chat_parser.o
...
ZAI_CODING_PROVIDER_SRC = src/zai_coding_provider.c
ZAI_CODING_PROVIDER_OBJ = $(BUILD_DIR)/zai_coding_provider.o
MINIMAX_CODING_PROVIDER_SRC = src/minimax_coding_provider.c
MINIMAX_CODING_PROVIDER_OBJ = $(BUILD_DIR)/minimax_coding_provider.o
KIMI_OAUTH_SRC = src/kimi_oauth.c
KIMI_OAUTH_OBJ = $(BUILD_DIR)/kimi_oauth.o
KIMI_CODING_PLAN_PROVIDER_SRC = src/kimi_coding_plan_provider.c
KIMI_CODING_PLAN_PROVIDER_OBJ = $(BUILD_DIR)/kimi_coding_plan_provider.o
OPENAI_OAUTH_SRC = src/openai_oauth.c
OPENAI_OAUTH_OBJ = $(BUILD_DIR)/openai_oauth.o
OPENAI_SUB_PROVIDER_SRC = src/openai_sub_provider.c
OPENAI_SUB_PROVIDER_OBJ = $(BUILD_DIR)/openai_sub_provider.o
ANTHROPIC_OAUTH_SRC = src/anthropic_oauth.c
ANTHROPIC_OAUTH_OBJ = $(BUILD_DIR)/anthropic_oauth.o
ANTHROPIC_SUB_PROVIDER_SRC = src/anthropic_sub_provider.c
ANTHROPIC_SUB_PROVIDER_OBJ = $(BUILD_DIR)/anthropic_sub_provider.o

OPENAI_RESPONSES_PROVIDER_SRC = src/openai_responses_provider.c
OPENAI_RESPONSES_PROVIDER_OBJ = $(BUILD_DIR)/openai_responses_provider.o
...
CRASH_HANDLER_SRC = src/crash_handler.c
CRASH_HANDLER_OBJ = $(BUILD_DIR)/crash_handler.o
AI_WORKER_SRC = src/ai_worker.c
AI_WORKER_OBJ = $(BUILD_DIR)/ai_worker.o
VOICE_INPUT_OBJ = $(BUILD_DIR)/voice_input.o
VOICE_TRANSCRIBER_SRC = src/voice_transcriber.c
VOICE_TRANSCRIBER_OBJ = $(BUILD_DIR)/voice_transcriber.o
OPENAI_TRANSCRIBE_SRC = src/openai_transcribe.c
OPENAI_TRANSCRIBE_OBJ = $(BUILD_DIR)/openai_transcribe.o
VOICE_MODE_SRC = src/voice_mode.c
VOICE_MODE_OBJ = $(BUILD_DIR)/voice_mode.o
...
SSE_PARSER_SRC = src/sse_parser.c
SSE_PARSER_OBJ = $(BUILD_DIR)/sse_parser.o
STREAMING_TOOL_ACCUMULATOR_SRC = src/streaming_tool_accumulator.c
STREAMING_TOOL_ACCUMULATOR_OBJ = $(BUILD_DIR)/streaming_tool_accumulator.o
...
REDACT_UTILS_SRC = src/util/redact_utils.c
REDACT_UTILS_OBJ = $(BUILD_DIR)/redact_utils.o
MODEL_CAPABILITIES_SRC = src/model_capabilities.c
MODEL_CAPABILITIES_OBJ = $(BUILD_DIR)/model_capabilities.o
# Tool modules
...
CODEX_TOOLS_SRC = src/tools/codex_tools.c
CODEX_TOOLS_OBJ = $(BUILD_DIR)/codex_tools.o
DYNAMIC_TOOLS_SRC = src/dynamic_tools.c
DYNAMIC_TOOLS_OBJ = $(BUILD_DIR)/dynamic_tools.o
...
CONVERSATION_PROCESSOR_SRC = src/conversation/conversation_processor.c
CONVERSATION_PROCESSOR_OBJ = $(BUILD_DIR)/conversation_processor.o
...
ONESHOT_UI_SRC = src/oneshot/oneshot_ui.c
ONESHOT_UI_OBJ = $(BUILD_DIR)/oneshot_ui.o

# Perpetual mode modules
PERPETUAL_LOG_SRC = src/perpetual/perpetual_log.c
PERPETUAL_LOG_OBJ = $(BUILD_DIR)/perpetual_log.o
PERPETUAL_PROMPT_SRC = src/perpetual/perpetual_prompt.c
PERPETUAL_PROMPT_OBJ = $(BUILD_DIR)/perpetual_prompt.o
PERPETUAL_MODE_SRC = src/perpetual/perpetual_mode.c
PERPETUAL_MODE_OBJ = $(BUILD_DIR)/perpetual_mode.o
...
TEST_FIREWORKS_TOKEN_FALLBACK_SRC = tests/test_fireworks_token_fallback.c
TEST_FIREWORKS_TOKEN_FALLBACK_TARGET = $(BUILD_DIR)/test_fireworks_token_fallback
...
TEST_TOKEN_USAGE_SESSION_TOTALS_SRC = tests/test_token_usage_session_totals.c
TEST_SESSION_RESOLVE_TARGET = $(BUILD_DIR)/test_session_resolve
TEST_SESSION_RESOLVE_SRC = tests/test_session_resolve.c
...
test-command-palette: check-deps $(TEST_COMMAND_PALETTE_TARGET)
	@echo ""
	@echo "Running Command Palette tests..."
	@echo ""
	@./$(TEST_COMMAND_PALETTE_TARGET)
...
test-tui-streaming-index: check-deps $(TEST_TUI_STREAMING_INDEX_TARGET)
	@echo ""
...
test-memory-db: check-deps $(TEST_MEMORY_DB_TARGET)
	@echo ""
...
test-session-resolve: check-deps $(TEST_SESSION_RESOLVE_TARGET)
	@echo ""
	@echo "Running Session Resolve tests..."
	@echo ""
	@./$(TEST_SESSION_RESOLVE_TARGET)
...
test-fireworks-token-fallback: check-deps $(TEST_FIREWORKS_TOKEN_FALLBACK_TARGET)
	@echo ""
	@echo "Running Fireworks Token Fallback tests..."
	@echo ""
	@./$(TEST_FIREWORKS_TOKEN_FALLBACK_TARGET)

test-sse-parser: check-deps $(TEST_SSE_PARSER_TARGET)
	@echo ""
	@echo "Running SSE Parser tests..."
	@echo ""
	@./$(TEST_SSE_PARSER_TARGET)

test-streaming-tool-accumulator: check-deps $(TEST_STREAMING_TOOL_ACCUMULATOR_TARGET)
	@echo ""
	@echo "Running Streaming Tool Accumulator tests..."
	@echo ""
	@./$(TEST_STREAMING_TOOL_ACCUMULATOR_TARGET)
...
test-markdown-render: check-deps $(TEST_MARKDOWN_RENDER_TARGET)
	@echo ""
	@echo "Running Markdown table detection tests..."
	@echo ""
...
$(TARGET): $(SRC) $(LOGGER_OBJ) $(CONFIG_OBJ) $(PERSISTENCE_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(TOKEN_USAGE_DB_OBJ) $(TOKEN_USAGE_DB_MIGRATIONS_OBJ) $(MIGRATIONS_OBJ) $(COMMANDS_OBJ) $(THEME_EXPLORER_OBJ) $(HELP_MODAL_OBJ) $(SETTINGS_MENU_OBJ) $(COMPLETION_OBJ) $(TUI_OBJ) $(TUI_CORE_OBJ) $(TUI_CONVERSATION_OBJ) $(TUI_WINDOW_OBJ) $(TUI_RENDER_OBJ) $(LINE_PRINTER_OBJ) $(MARKDOWN_RENDER_OBJ) $(TUI_MODES_OBJ) $(TUI_INPUT_OBJ) $(TUI_HISTORY_OBJ) $(TUI_COMPLETION_OBJ) $(TUI_PASTE_OBJ) $(TUI_SEARCH_OBJ) $(VLTRN_BANNER_OBJ) $(FILE_SEARCH_OBJ) $(HISTORY_SEARCH_OBJ) $(WINDOW_MANAGER_OBJ) $(TODO_OBJ) $(GOAL_OBJ) $(AWS_BEDROCK_OBJ) $(BEDROCK_CONVERSE_OBJ) $(PROVIDER_OBJ) $(PROVIDER_CONFIG_LOADER_OBJ) $(OPENAI_PROVIDER_OBJ) $(OPENAI_STREAMING_OBJ) $(OPENAI_CHAT_PARSER_OBJ) $(OPENAI_MESSAGES_OBJ) $(OPENAI_RESPONSES_OBJ) $(BEDROCK_PROVIDER_OBJ) $(ANTHROPIC_PROVIDER_OBJ) $(DEEPSEEK_PROVIDER_OBJ) $(MOONSHOT_PROVIDER_OBJ) $(ZAI_CODING_PROVIDER_OBJ) $(MINIMAX_CODING_PROVIDER_OBJ) $(KIMI_OAUTH_OBJ) $(KIMI_CODING_PLAN_PROVIDER_OBJ) $(OPENAI_OAUTH_OBJ) $(OPENAI_SUB_PROVIDER_OBJ) $(ANTHROPIC_OAUTH_OBJ) $(ANTHROPIC_SUB_PROVIDER_OBJ) $(OPENAI_RESPONSES_PROVIDER_OBJ) $(BUILTIN_THEMES_OBJ) $(MESSAGE_QUEUE_OBJ) $(AI_WORKER_OBJ) $(CRASH_HANDLER_OBJ) $(VOICE_INPUT_OBJ) $(VOICE_TRANSCRIBER_OBJ) $(OPENAI_TRANSCRIBE_OBJ) $(VOICE_MODE_OBJ) $(SQLITE_QUEUE_OBJ) $(MCP_OBJ) $(TOOL_UTILS_OBJ) $(PROCESS_UTILS_OBJ) $(DUMP_UTILS_OBJ) $(SUBAGENT_MANAGER_OBJ) $(EXPLORE_TOOLS_OBJ) $(BASE64_OBJ) $(HISTORY_FILE_OBJ) $(ARRAY_RESIZE_OBJ) $(HTTP_CLIENT_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(SESSION_OBJ) $(SESSION_RESOLVE_OBJ) $(RETRY_LOGIC_OBJ) $(MEMORY_DB_OBJ) $(COMPACTION_OBJ) $(BACKGROUND_INIT_OBJ) $(DATA_DIR_OBJ) $(FILE_UTILS_OBJ) $(STRING_UTILS_OBJ) $(TIMESTAMP_UTILS_OBJ) $(FORMAT_UTILS_OBJ) $(ENV_UTILS_OBJ) $(OUTPUT_UTILS_OBJ) $(DIFF_UTILS_OBJ) $(REDACT_UTILS_OBJ) $(MODEL_CAPABILITIES_OBJ) $(TOOL_SLEEP_OBJ) $(TOOL_TODO_OBJ) $(TOOL_IMAGE_OBJ) $(TOOL_SEARCH_OBJ) $(TOOL_FILESYSTEM_OBJ) $(TOOL_BASH_OBJ) $(TOOL_SUBAGENT_OBJ) $(CODEX_TOOLS_OBJ) $(TOOL_REGISTRY_OBJ) $(TOOL_EXECUTOR_OBJ) $(TOOL_DEFINITIONS_OBJ) $(DYNAMIC_TOOLS_OBJ) $(CONTENT_TYPES_OBJ) $(CONVERSATION_STATE_OBJ) $(MESSAGE_BUILDER_OBJ) $(MESSAGE_PARSER_OBJ) $(CONVERSATION_PROCESSOR_OBJ) $(CONTEXT_SYSTEM_PROMPT_OBJ) $(CONTEXT_ENVIRONMENT_OBJ) $(CONTEXT_KLAWED_MD_OBJ) $(CONTEXT_MEMORY_INJECTION_OBJ) $(UI_OUTPUT_OBJ) $(PRINT_HELPERS_OBJ) $(TOOL_OUTPUT_DISPLAY_OBJ) $(SESSION_TOKEN_USAGE_OBJ) $(SESSION_PERSISTENCE_OBJ) $(API_RESPONSE_OBJ) $(API_BUILDER_OBJ) $(API_CLIENT_OBJ) $(INTERACTIVE_LOOP_OBJ) $(INPUT_HANDLER_OBJ) $(RESPONSE_PROCESSOR_OBJ) $(COMMAND_DISPATCH_OBJ) $(ONESHOT_MODE_OBJ) $(ONESHOT_PROCESSOR_OBJ) $(ONESHOT_OUTPUT_OBJ) $(ONESHOT_UI_OBJ) $(PROVIDER_COMMAND_OBJ) $(CONFIG_COMMAND_OBJ) $(PERPETUAL_LOG_OBJ) $(PERPETUAL_PROMPT_OBJ) $(PERPETUAL_MODE_OBJ) $(VERSION_H)
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -o $(TARGET) $(SRC) $(LOGGER_OBJ) $(CONFIG_OBJ) $(PERSISTENCE_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(TOKEN_USAGE_DB_OBJ) $(TOKEN_USAGE_DB_MIGRATIONS_OBJ) $(MIGRATIONS_OBJ) $(COMMANDS_OBJ) $(THEME_EXPLORER_OBJ) $(HELP_MODAL_OBJ) $(SETTINGS_MENU_OBJ) $(COMPLETION_OBJ) $(TUI_OBJ) $(TUI_CORE_OBJ) $(TUI_CONVERSATION_OBJ) $(TUI_WINDOW_OBJ) $(TUI_RENDER_OBJ) $(LINE_PRINTER_OBJ) $(MARKDOWN_RENDER_OBJ) $(TUI_MODES_OBJ) $(TUI_INPUT_OBJ) $(TUI_HISTORY_OBJ) $(TUI_COMPLETION_OBJ) $(TUI_PASTE_OBJ) $(TUI_SEARCH_OBJ) $(VLTRN_BANNER_OBJ) $(FILE_SEARCH_OBJ) $(HISTORY_SEARCH_OBJ) $(WINDOW_MANAGER_OBJ) $(TODO_OBJ) $(GOAL_OBJ) $(AWS_BEDROCK_OBJ) $(BEDROCK_CONVERSE_OBJ) $(PROVIDER_OBJ) $(PROVIDER_CONFIG_LOADER_OBJ) $(OPENAI_PROVIDER_OBJ) $(OPENAI_STREAMING_OBJ) $(OPENAI_CHAT_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_MESSAGES_OBJ) $(OPENAI_RESPONSES_OBJ) $(BEDROCK_PROVIDER_OBJ) $(ANTHROPIC_PROVIDER_OBJ) $(DEEPSEEK_PROVIDER_OBJ) $(MOONSHOT_PROVIDER_OBJ) $(ZAI_CODING_PROVIDER_OBJ) $(MINIMAX_CODING_PROVIDER_OBJ) $(KIMI_OAUTH_OBJ) $(KIMI_CODING_PLAN_PROVIDER_OBJ) $(OPENAI_OAUTH_OBJ) $(OPENAI_SUB_PROVIDER_OBJ) $(ANTHROPIC_OAUTH_OBJ) $(ANTHROPIC_SUB_PROVIDER_OBJ) $(OPENAI_RESPONSES_PROVIDER_OBJ) $(BUILTIN_THEMES_OBJ) $(MESSAGE_QUEUE_OBJ) $(AI_WORKER_OBJ) $(CRASH_HANDLER_OBJ) $(VOICE_INPUT_OBJ) $(VOICE_TRANSCRIBER_OBJ) $(OPENAI_TRANSCRIBE_OBJ) $(VOICE_MODE_OBJ) $(SQLITE_QUEUE_OBJ) $(MCP_OBJ) $(TOOL_UTILS_OBJ) $(PROCESS_UTILS_OBJ) $(DUMP_UTILS_OBJ) $(SUBAGENT_MANAGER_OBJ) $(EXPLORE_TOOLS_OBJ) $(BASE64_OBJ) $(HISTORY_FILE_OBJ) $(ARRAY_RESIZE_OBJ) $(HTTP_CLIENT_OBJ) $(SSE_PARSER_OBJ) $(SESSION_OBJ) $(SESSION_RESOLVE_OBJ) $(RETRY_LOGIC_OBJ) $(MEMORY_DB_OBJ) $(COMPACTION_OBJ) $(BACKGROUND_INIT_OBJ) $(DATA_DIR_OBJ) $(FILE_UTILS_OBJ) $(STRING_UTILS_OBJ) $(TIMESTAMP_UTILS_OBJ) $(FORMAT_UTILS_OBJ) $(ENV_UTILS_OBJ) $(OUTPUT_UTILS_OBJ) $(DIFF_UTILS_OBJ) $(REDACT_UTILS_OBJ) $(MODEL_CAPABILITIES_OBJ) $(TOOL_SLEEP_OBJ) $(TOOL_TODO_OBJ) $(TOOL_IMAGE_OBJ) $(TOOL_SEARCH_OBJ) $(TOOL_FILESYSTEM_OBJ) $(TOOL_BASH_OBJ) $(TOOL_SUBAGENT_OBJ) $(CODEX_TOOLS_OBJ) $(TOOL_REGISTRY_OBJ) $(TOOL_EXECUTOR_OBJ) $(TOOL_DEFINITIONS_OBJ) $(DYNAMIC_TOOLS_OBJ) $(CONTENT_TYPES_OBJ) $(CONVERSATION_STATE_OBJ) $(MESSAGE_BUILDER_OBJ) $(MESSAGE_PARSER_OBJ) $(CONVERSATION_PROCESSOR_OBJ) $(CONTEXT_SYSTEM_PROMPT_OBJ) $(CONTEXT_ENVIRONMENT_OBJ) $(CONTEXT_KLAWED_MD_OBJ) $(CONTEXT_MEMORY_INJECTION_OBJ) $(UI_OUTPUT_OBJ) $(PRINT_HELPERS_OBJ) $(TOOL_OUTPUT_DISPLAY_OBJ) $(SESSION_TOKEN_USAGE_OBJ) $(SESSION_PERSISTENCE_OBJ) $(API_RESPONSE_OBJ) $(API_BUILDER_OBJ) $(API_CLIENT_OBJ) $(INTERACTIVE_LOOP_OBJ) $(INPUT_HANDLER_OBJ) $(RESPONSE_PROCESSOR_OBJ) $(COMMAND_DISPATCH_OBJ) $(ONESHOT_MODE_OBJ) $(ONESHOT_PROCESSOR_OBJ) $(ONESHOT_OUTPUT_OBJ) $(ONESHOT_UI_OBJ) $(PROVIDER_COMMAND_OBJ) $(CONFIG_COMMAND_OBJ) $(PERPETUAL_LOG_OBJ) $(PERPETUAL_PROMPT_OBJ) $(PERPETUAL_MODE_OBJ) $(LDFLAGS)
	@echo ""
...
$(BUILD_DIR)/http_client.o: src/http_client.c src/http_client.h src/sse_parser.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/http_client.o src/http_client.c

# Build SSE parser object
$(BUILD_DIR)/sse_parser.o: src/sse_parser.c src/sse_parser.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/sse_parser.o src/sse_parser.c

# Build streaming tool accumulator object
$(BUILD_DIR)/streaming_tool_accumulator.o: src/streaming_tool_accumulator.c src/streaming_tool_accumulator.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/streaming_tool_accumulator.o src/streaming_tool_accumulator.c

# Build crash handler object
$(BUILD_DIR)/crash_handler.o: src/crash_handler.c src/crash_handler.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/crash_handler.o src/crash_handler.c
...
# Build shared OpenAI chat parser object
$(BUILD_DIR)/openai_chat_parser.o: $(OPENAI_CHAT_PARSER_SRC) src/openai_chat_parser.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/openai_chat_parser.o $(OPENAI_CHAT_PARSER_SRC)
...
$(BUILD_DIR)/session_resolve.o: src/session_resolve.c src/session_resolve.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/session_resolve.o src/session_resolve.c

$(BUILD_DIR)/retry_logic.o: src/retry_logic.c src/retry_logic.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/retry_logic.o src/retry_logic.c

# Build macOS SQLite fix object
$(BUILD_DIR)/macos_sqlite_fix.o: $(MACOS_SQLITE_FIX_SRC) src/macos_sqlite_fix.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/macos_sqlite_fix.o $(MACOS_SQLITE_FIX_SRC)
...
# Build compaction object (context compaction)
...
$(BUILD_DIR)/redact_utils.o: $(REDACT_UTILS_SRC) src/util/redact_utils.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/redact_utils.o $(REDACT_UTILS_SRC)

$(BUILD_DIR)/model_capabilities.o: $(MODEL_CAPABILITIES_SRC) src/model_capabilities.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/model_capabilities.o $(MODEL_CAPABILITIES_SRC)
...
$(BUILD_DIR)/codex_tools.o: $(CODEX_TOOLS_SRC) src/tools/codex_tools.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/codex_tools.o $(CODEX_TOOLS_SRC)

$(BUILD_DIR)/dynamic_tools.o: $(DYNAMIC_TOOLS_SRC) src/dynamic_tools.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/dynamic_tools.o $(DYNAMIC_TOOLS_SRC)
...
$(BUILD_DIR)/conversation_processor.o: $(CONVERSATION_PROCESSOR_SRC) src/conversation/conversation_processor.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/conversation_processor.o $(CONVERSATION_PROCESSOR_SRC)
...
$(BUILD_DIR)/oneshot_ui.o: $(ONESHOT_UI_SRC) src/oneshot/oneshot_ui.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/oneshot_ui.o $(ONESHOT_UI_SRC)

# Build perpetual mode objects
$(BUILD_DIR)/perpetual_log.o: $(PERPETUAL_LOG_SRC) src/perpetual/perpetual_log.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/perpetual_log.o $(PERPETUAL_LOG_SRC)

$(BUILD_DIR)/perpetual_prompt.o: $(PERPETUAL_PROMPT_SRC) src/perpetual/perpetual_prompt.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/perpetual_prompt.o $(PERPETUAL_PROMPT_SRC)

$(BUILD_DIR)/perpetual_mode.o: $(PERPETUAL_MODE_SRC) src/perpetual/perpetual_mode.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/perpetual_mode.o $(PERPETUAL_MODE_SRC)
...
$(BUILD_DIR)/klawed-debug: $(SRC) $(LOGGER_SRC) $(PERSISTENCE_SRC) $(MIGRATIONS_SRC) $(COMMANDS_SRC) $(COMPLETION_SRC) $(TUI_SRC) $(WINDOW_MANAGER_SRC) $(TODO_SRC) $(AWS_BEDROCK_SRC) $(PROVIDER_SRC) $(OPENAI_PROVIDER_SRC) $(OPENAI_CHAT_PARSER_SRC) $(OPENAI_MESSAGES_SRC) $(OPENAI_RESPONSES_SRC) $(BEDROCK_PROVIDER_SRC) $(ANTHROPIC_PROVIDER_SRC) $(BUILTIN_THEMES_SRC) $(MESSAGE_QUEUE_SRC) $(AI_WORKER_SRC) $(VOICE_INPUT_SRC) $(ZMQ_SOCKET_SRC) $(ZMQ_CLIENT_SRC) $(ZMQ_MESSAGE_QUEUE_SRC) $(ZMQ_DAEMON_SRC) $(SQLITE_QUEUE_SRC) $(MCP_SRC) $(TOOL_UTILS_SRC) $(PROCESS_UTILS_SRC) $(SUBAGENT_MANAGER_SRC) $(BASE64_SRC) $(HISTORY_FILE_SRC) $(ARRAY_RESIZE_SRC) $(HTTP_CLIENT_SRC) $(SESSION_SRC) $(RETRY_LOGIC_SRC)
	...
	$(CC) $(DEBUG_CFLAGS) -c -o $(BUILD_DIR)/openai_chat_parser_debug.o $(OPENAI_CHAT_PARSER_SRC)
...
$(BUILD_DIR)/klawed-clang: $(SRC) $(LOGGER_OBJ) $(PERSISTENCE_OBJ) $(TOKEN_USAGE_DB_OBJ) $(TOKEN_USAGE_DB_MIGRATIONS_OBJ) $(MIGRATIONS_OBJ) $(COMMANDS_OBJ) $(COMPLETION_OBJ) $(TUI_OBJ) $(WINDOW_MANAGER_OBJ) $(TODO_OBJ) $(GOAL_OBJ) $(AWS_BEDROCK_OBJ) $(BEDROCK_CONVERSE_OBJ) $(PROVIDER_OBJ) $(OPENAI_PROVIDER_OBJ) $(OPENAI_CHAT_PARSER_OBJ) $(OPENAI_MESSAGES_OBJ) $(OPENAI_RESPONSES_OBJ) $(BEDROCK_PROVIDER_OBJ) $(ANTHROPIC_PROVIDER_OBJ) $(DEEPSEEK_PROVIDER_OBJ) $(MOONSHOT_PROVIDER_OBJ) $(ZAI_CODING_PROVIDER_OBJ) $(MINIMAX_CODING_PROVIDER_OBJ) $(BUILTIN_THEMES_OBJ) $(AI_WORKER_OBJ) $(MESSAGE_QUEUE_OBJ) $(CRASH_HANDLER_OBJ) $(VOICE_INPUT_OBJ) $(VOICE_TRANSCRIBER_OBJ) $(OPENAI_TRANSCRIBE_OBJ) $(VOICE_MODE_OBJ) $(SQLITE_QUEUE_OBJ) $(MCP_OBJ) $(TOOL_UTILS_SRC) $(PROCESS_UTILS_OBJ) $(HTTP_CLIENT_OBJ) $(RETRY_LOGIC_OBJ) $(VERSION_H)
	@mkdir -p $(BUILD_DIR)
	@echo "Building with clang compiler..."
	$(CLANG) $(CFLAGS) -o $(BUILD_DIR)/klawed-clang $(SRC) $(LOGGER_OBJ) $(PERSISTENCE_OBJ) $(TOKEN_USAGE_DB_OBJ) $(TOKEN_USAGE_DB_MIGRATIONS_OBJ) $(MIGRATIONS_OBJ) $(COMMANDS_OBJ) $(COMPLETION_OBJ) $(TUI_OBJ) $(WINDOW_MANAGER_OBJ) $(TODO_OBJ) $(GOAL_OBJ) $(AWS_BEDROCK_OBJ) $(BEDROCK_CONVERSE_OBJ) $(PROVIDER_OBJ) $(OPENAI_PROVIDER_OBJ) $(OPENAI_CHAT_PARSER_OBJ) $(OPENAI_MESSAGES_OBJ) $(OPENAI_RESPONSES_OBJ) $(BEDROCK_PROVIDER_OBJ) $(ANTHROPIC_PROVIDER_OBJ) $(DEEPSEEK_PROVIDER_OBJ) $(MOONSHOT_PROVIDER_OBJ) $(ZAI_CODING_PROVIDER_OBJ) $(MINIMAX_CODING_PROVIDER_OBJ) $(BUILTIN_THEMES_OBJ) $(MESSAGE_QUEUE_OBJ) $(AI_WORKER_OBJ) $(CRASH_HANDLER_OBJ) $(VOICE_INPUT_OBJ) $(VOICE_TRANSCRIBER_OBJ) $(OPENAI_TRANSCRIBE_OBJ) $(VOICE_MODE_OBJ) $(SQLITE_QUEUE_OBJ) $(MCP_OBJ) $(TOOL_UTILS_SRC) $(PROCESS_UTILS_OBJ) $(HTTP_CLIENT_OBJ) $(RETRY_LOGIC_OBJ) $(LDFLAGS)
	@echo ""
...
sanitize-ub: check-deps $(LOGGER_OBJ) $(CONFIG_OBJ) $(PERSISTENCE_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(TOKEN_USAGE_DB_OBJ) $(TOKEN_USAGE_DB_MIGRATIONS_OBJ) $(MIGRATIONS_OBJ) $(COMMANDS_OBJ) $(THEME_EXPLORER_OBJ) $(HELP_MODAL_OBJ) $(SETTINGS_MENU_OBJ) $(COMPLETION_OBJ) $(TUI_OBJ) $(TUI_CORE_OBJ) $(TUI_CONVERSATION_OBJ) $(TUI_WINDOW_OBJ) $(TUI_RENDER_OBJ) $(LINE_PRINTER_OBJ) $(MARKDOWN_RENDER_OBJ) $(TUI_MODES_OBJ) $(TUI_INPUT_OBJ) $(TUI_HISTORY_OBJ) $(TUI_COMPLETION_OBJ) $(TUI_PASTE_OBJ) $(TUI_SEARCH_OBJ) $(VLTRN_BANNER_OBJ) $(FILE_SEARCH_OBJ) $(HISTORY_SEARCH_OBJ) $(WINDOW_MANAGER_OBJ) $(TODO_OBJ) $(GOAL_OBJ) $(AWS_BEDROCK_OBJ) $(BEDROCK_CONVERSE_OBJ) $(PROVIDER_OBJ) $(PROVIDER_CONFIG_LOADER_OBJ) $(OPENAI_PROVIDER_OBJ) $(OPENAI_STREAMING_OBJ) $(OPENAI_CHAT_PARSER_OBJ) $(OPENAI_MESSAGES_OBJ) $(OPENAI_RESPONSES_OBJ) $(BEDROCK_PROVIDER_OBJ) $(ANTHROPIC_PROVIDER_OBJ) $(DEEPSEEK_PROVIDER_OBJ) $(MOONSHOT_PROVIDER_OBJ) $(ZAI_CODING_PROVIDER_OBJ) $(MINIMAX_CODING_PROVIDER_OBJ) $(KIMI_OAUTH_OBJ) $(KIMI_CODING_PLAN_PROVIDER_OBJ) $(OPENAI_OAUTH_OBJ) $(OPENAI_SUB_PROVIDER_OBJ) $(ANTHROPIC_OAUTH_OBJ) $(ANTHROPIC_SUB_PROVIDER_OBJ) $(OPENAI_RESPONSES_PROVIDER_OBJ) $(BUILTIN_THEMES_OBJ) $(MESSAGE_QUEUE_OBJ) $(AI_WORKER_OBJ) $(CRASH_HANDLER_OBJ) $(VOICE_INPUT_OBJ) $(VOICE_TRANSCRIBER_OBJ) $(OPENAI_TRANSCRIBE_OBJ) $(VOICE_MODE_OBJ) $(SQLITE_QUEUE_OBJ) $(MCP_OBJ) $(TOOL_UTILS_OBJ) $(PROCESS_UTILS_OBJ) $(DUMP_UTILS_OBJ) $(SUBAGENT_MANAGER_OBJ) $(EXPLORE_TOOLS_OBJ) $(BASE64_OBJ) $(HISTORY_FILE_OBJ) $(ARRAY_RESIZE_OBJ) $(HTTP_CLIENT_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(SESSION_OBJ) $(SESSION_RESOLVE_OBJ) $(RETRY_LOGIC_OBJ) $(MEMORY_DB_OBJ) $(COMPACTION_OBJ) $(BACKGROUND_INIT_OBJ) $(DATA_DIR_OBJ) $(FILE_UTILS_OBJ) $(STRING_UTILS_OBJ) $(TIMESTAMP_UTILS_OBJ) $(FORMAT_UTILS_OBJ) $(ENV_UTILS_OBJ) $(OUTPUT_UTILS_OBJ) $(DIFF_UTILS_OBJ) $(REDACT_UTILS_OBJ) $(MODEL_CAPABILITIES_OBJ) $(TOOL_SLEEP_OBJ) $(TOOL_TODO_OBJ) $(TOOL_IMAGE_OBJ) $(TOOL_SEARCH_OBJ) $(TOOL_FILESYSTEM_OBJ) $(TOOL_BASH_OBJ) $(TOOL_SUBAGENT_OBJ) $(TOOL_REGISTRY_OBJ) $(TOOL_EXECUTOR_OBJ) $(TOOL_DEFINITIONS_OBJ) $(DYNAMIC_TOOLS_OBJ) $(CONTENT_TYPES_OBJ) $(CONVERSATION_STATE_OBJ) $(MESSAGE_BUILDER_OBJ) $(MESSAGE_PARSER_OBJ) $(CONVERSATION_PROCESSOR_OBJ) $(CONTEXT_SYSTEM_PROMPT_OBJ) $(CONTEXT_ENVIRONMENT_OBJ) $(CONTEXT_KLAWED_MD_OBJ) $(CONTEXT_MEMORY_INJECTION_OBJ) $(UI_OUTPUT_OBJ) $(PRINT_HELPERS_OBJ) $(TOOL_OUTPUT_DISPLAY_OBJ) $(SESSION_TOKEN_USAGE_OBJ) $(SESSION_PERSISTENCE_OBJ) $(API_RESPONSE_OBJ) $(API_BUILDER_OBJ) $(API_CLIENT_OBJ) $(INTERACTIVE_LOOP_OBJ) $(INPUT_HANDLER_OBJ) $(RESPONSE_PROCESSOR_OBJ) $(COMMAND_DISPATCH_OBJ) $(ONESHOT_MODE_OBJ) $(ONESHOT_PROCESSOR_OBJ) $(ONESHOT_OUTPUT_OBJ) $(ONESHOT_UI_OBJ) $(PROVIDER_COMMAND_OBJ) $(CONFIG_COMMAND_OBJ) $(PERPETUAL_LOG_OBJ) $(PERPETUAL_PROMPT_OBJ) $(PERPETUAL_MODE_OBJ)
	@mkdir -p $(BUILD_DIR)
	@echo "Building with Undefined Behavior Sanitizer..."
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=undefined -fno-omit-frame-pointer -o $(BUILD_DIR)/klawed-ubsan $(SRC) $(LOGGER_OBJ) $(CONFIG_OBJ) $(PERSISTENCE_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(TOKEN_USAGE_DB_OBJ) $(TOKEN_USAGE_DB_MIGRATIONS_OBJ) $(MIGRATIONS_OBJ) $(COMMANDS_OBJ) $(THEME_EXPLORER_OBJ) $(HELP_MODAL_OBJ) $(SETTINGS_MENU_OBJ) $(COMPLETION_OBJ) $(TUI_OBJ) $(TUI_CORE_OBJ) $(TUI_CONVERSATION_OBJ) $(TUI_WINDOW_OBJ) $(TUI_RENDER_OBJ) $(LINE_PRINTER_OBJ) $(TUI_MODES_OBJ) $(TUI_INPUT_OBJ) $(TUI_HISTORY_OBJ) $(TUI_COMPLETION_OBJ) $(TUI_PASTE_OBJ) $(TUI_SEARCH_OBJ) $(VLTRN_BANNER_OBJ) $(FILE_SEARCH_OBJ) $(HISTORY_SEARCH_OBJ) $(WINDOW_MANAGER_OBJ) $(TODO_OBJ) $(GOAL_OBJ) $(AWS_BEDROCK_OBJ) $(BEDROCK_CONVERSE_OBJ) $(PROVIDER_OBJ) $(PROVIDER_CONFIG_LOADER_OBJ) $(OPENAI_PROVIDER_OBJ) $(OPENAI_STREAMING_OBJ) $(OPENAI_CHAT_PARSER_OBJ) $(OPENAI_MESSAGES_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_RESPONSES_OBJ) $(BEDROCK_PROVIDER_OBJ) $(ANTHROPIC_PROVIDER_OBJ) $(DEEPSEEK_PROVIDER_OBJ) $(MOONSHOT_PROVIDER_OBJ) $(ZAI_CODING_PROVIDER_OBJ) $(MINIMAX_CODING_PROVIDER_OBJ) $(KIMI_OAUTH_OBJ) $(KIMI_CODING_PLAN_PROVIDER_OBJ) $(OPENAI_OAUTH_OBJ) $(OPENAI_SUB_PROVIDER_OBJ) $(ANTHROPIC_OAUTH_OBJ) $(ANTHROPIC_SUB_PROVIDER_OBJ) $(OPENAI_RESPONSES_PROVIDER_OBJ) $(BUILTIN_THEMES_OBJ) $(MESSAGE_QUEUE_OBJ) $(AI_WORKER_OBJ) $(CRASH_HANDLER_OBJ) $(VOICE_INPUT_OBJ) $(VOICE_TRANSCRIBER_OBJ) $(OPENAI_TRANSCRIBE_OBJ) $(VOICE_MODE_OBJ) $(SQLITE_QUEUE_OBJ) $(MCP_OBJ) $(TOOL_UTILS_OBJ) $(PROCESS_UTILS_OBJ) $(DUMP_UTILS_OBJ) $(SUBAGENT_MANAGER_OBJ) $(EXPLORE_TOOLS_OBJ) $(BASE64_OBJ) $(HISTORY_FILE_OBJ) $(ARRAY_RESIZE_OBJ) $(HTTP_CLIENT_OBJ) $(SSE_PARSER_OBJ) $(SESSION_OBJ) $(SESSION_RESOLVE_OBJ) $(RETRY_LOGIC_OBJ) $(MEMORY_DB_OBJ) $(COMPACTION_OBJ) $(BACKGROUND_INIT_OBJ) $(DATA_DIR_OBJ) $(FILE_UTILS_OBJ) $(STRING_UTILS_OBJ) $(TIMESTAMP_UTILS_OBJ) $(FORMAT_UTILS_OBJ) $(ENV_UTILS_OBJ) $(OUTPUT_UTILS_OBJ) $(DIFF_UTILS_OBJ) $(REDACT_UTILS_OBJ) $(MODEL_CAPABILITIES_OBJ) $(TOOL_SLEEP_OBJ) $(TOOL_TODO_OBJ) $(TOOL_IMAGE_OBJ) $(TOOL_SEARCH_OBJ) $(TOOL_FILESYSTEM_OBJ) $(TOOL_BASH_OBJ) $(TOOL_SUBAGENT_OBJ) $(TOOL_REGISTRY_OBJ) $(TOOL_EXECUTOR_OBJ) $(TOOL_DEFINITIONS_OBJ) $(DYNAMIC_TOOLS_OBJ) $(CONTENT_TYPES_OBJ) $(CONVERSATION_STATE_OBJ) $(MESSAGE_BUILDER_OBJ) $(MESSAGE_PARSER_OBJ) $(CONVERSATION_PROCESSOR_OBJ) $(CONTEXT_SYSTEM_PROMPT_OBJ) $(CONTEXT_ENVIRONMENT_OBJ) $(CONTEXT_KLAWED_MD_OBJ) $(CONTEXT_MEMORY_INJECTION_OBJ) $(UI_OUTPUT_OBJ) $(PRINT_HELPERS_OBJ) $(TOOL_OUTPUT_DISPLAY_OBJ) $(SESSION_TOKEN_USAGE_OBJ) $(SESSION_PERSISTENCE_OBJ) $(API_RESPONSE_OBJ) $(API_BUILDER_OBJ) $(API_CLIENT_OBJ) $(INTERACTIVE_LOOP_OBJ) $(INPUT_HANDLER_OBJ) $(RESPONSE_PROCESSOR_OBJ) $(COMMAND_DISPATCH_OBJ) $(ONESHOT_MODE_OBJ) $(ONESHOT_PROCESSOR_OBJ) $(ONESHOT_OUTPUT_OBJ) $(ONESHOT_UI_OBJ) $(PROVIDER_COMMAND_OBJ) $(CONFIG_COMMAND_OBJ) $(PERPETUAL_LOG_OBJ) $(PERPETUAL_PROMPT_OBJ) $(PERPETUAL_MODE_OBJ) $(LDFLAGS) -fsanitize=undefined
	@echo ""
...
sanitize-all: check-deps
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/crash_handler_all.o $(CRASH_HANDLER_SRC); \
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/settings_menu_all.o $(SETTINGS_MENU_SRC); \
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/line_printer_all.o $(LINE_PRINTER_SRC); \
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/vltrn_banner_all.o $(VLTRN_BANNER_SRC); \
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/goal_all.o $(GOAL_SRC); \
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/openai_chat_parser_all.o $(OPENAI_CHAT_PARSER_SRC); \
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/openai_streaming_all.o $(OPENAI_STREAMING_SRC); \
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/sse_parser_all.o $(SSE_PARSER_SRC); \
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/streaming_tool_accumulator_all.o $(STREAMING_TOOL_ACCUMULATOR_SRC); \
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/zai_coding_provider_all.o $(ZAI_CODING_PROVIDER_SRC); \
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/minimax_coding_provider_all.o $(MINIMAX_CODING_PROVIDER_SRC); \
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/openai_oauth_all.o $(OPENAI_OAUTH_SRC); \
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/openai_sub_provider_all.o $(OPENAI_SUB_PROVIDER_SRC); \
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/anthropic_oauth_all.o $(ANTHROPIC_OAUTH_SRC); \
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/anthropic_sub_provider_all.o $(ANTHROPIC_SUB_PROVIDER_SRC); \
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/openai_responses_provider_all.o $(OPENAI_RESPONSES_PROVIDER_SRC); \
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/voice_transcriber_all.o $(VOICE_TRANSCRIBER_SRC); \
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/openai_transcribe_all.o $(OPENAI_TRANSCRIBE_SRC); \
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/voice_mode_all.o $(VOICE_MODE_SRC); \
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/redact_utils_all.o $(REDACT_UTILS_SRC); \
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/model_capabilities_all.o $(MODEL_CAPABILITIES_SRC); \
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/codex_tools_all.o $(CODEX_TOOLS_SRC); \
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -c -o $(BUILD_DIR)/oneshot_ui_all.o $(ONESHOT_UI_SRC); \
	...
	$(CC) $(filter-out -O2 -D_FORTIFY_SOURCE=2,$(CFLAGS)) -g -O1 -fsanitize=address,undefined -fno-omit-frame-pointer $$EXTRA_FLAGS -o $(BUILD_DIR)/klawed-allsan $(SRC) \
		$(BUILD_DIR)/crash_handler_all.o $(BUILD_DIR)/logger_all.o $(BUILD_DIR)/persistence_all.o $(BUILD_DIR)/macos_sqlite_fix_all.o $(BUILD_DIR)/migrations_all.o \
		...
		$(BUILD_DIR)/tui_window_all.o $(BUILD_DIR)/tui_render_all.o $(BUILD_DIR)/markdown_render_all.o $(BUILD_DIR)/line_printer_all.o $(BUILD_DIR)/tui_modes_all.o \
		...
		$(BUILD_DIR)/openai_chat_parser_all.o \
		$(BUILD_DIR)/openai_streaming_all.o \
		$(BUILD_DIR)/sse_parser_all.o \
		$(BUILD_DIR)/streaming_tool_accumulator_all.o \
		...
		$(BUILD_DIR)/zai_coding_provider_all.o \
		$(BUILD_DIR)/minimax_coding_provider_all.o \
		$(BUILD_DIR)/kimi_coding_plan_provider_all.o \
		$(BUILD_DIR)/kimi_oauth_all.o \
		$(BUILD_DIR)/openai_oauth_all.o \
		$(BUILD_DIR)/openai_sub_provider_all.o \
		$(BUILD_DIR)/anthropic_oauth_all.o \
		$(BUILD_DIR)/anthropic_sub_provider_all.o \
		$(BUILD_DIR)/openai_responses_provider_all.o \
		$(BUILD_DIR)/builtin_themes_all.o \
		$(BUILD_DIR)/message_queue_all.o $(BUILD_DIR)/ai_worker_all.o \
		$(BUILD_DIR)/voice_input_all.o \
		$(BUILD_DIR)/voice_transcriber_all.o \
		$(BUILD_DIR)/openai_transcribe_all.o \
		$(BUILD_DIR)/voice_mode_all.o \
...
# Note: Using -O1 instead of the regular -O2, and removing _FORTIFY_SOURCE to avoid conflicts
sanitize-leak: check-deps $(LOGGER_OBJ) $(CONFIG_OBJ) $(PERSISTENCE_OBJ) $(TOKEN_USAGE_DB_OBJ) $(TOKEN_USAGE_DB_MIGRATIONS_OBJ) $(MIGRATIONS_OBJ) $(COMMANDS_OBJ) $(THEME_EXPLORER_OBJ) $(HELP_MODAL_OBJ) $(COMPLETION_OBJ) $(TUI_OBJ) $(TUI_CORE_OBJ) $(TUI_CONVERSATION_OBJ) $(TUI_WINDOW_OBJ) $(TUI_RENDER_OBJ) $(LINE_PRINTER_OBJ) $(TUI_MODES_OBJ) $(TUI_INPUT_OBJ) $(TUI_HISTORY_OBJ) $(TUI_COMPLETION_OBJ) $(TUI_PASTE_OBJ) $(TUI_SEARCH_OBJ) $(FILE_SEARCH_OBJ) $(HISTORY_SEARCH_OBJ) $(WINDOW_MANAGER_OBJ) $(TODO_OBJ) $(GOAL_OBJ) $(AWS_BEDROCK_OBJ) $(BEDROCK_CONVERSE_OBJ) $(PROVIDER_OBJ) $(PROVIDER_CONFIG_LOADER_OBJ) $(OPENAI_PROVIDER_OBJ) $(OPENAI_MESSAGES_OBJ) $(OPENAI_RESPONSES_OBJ) $(BEDROCK_PROVIDER_OBJ) $(ANTHROPIC_PROVIDER_OBJ) $(DEEPSEEK_PROVIDER_OBJ) $(MOONSHOT_PROVIDER_OBJ) $(ZAI_CODING_PROVIDER_OBJ) $(MINIMAX_CODING_PROVIDER_OBJ) $(BUILTIN_THEMES_OBJ) $(MESSAGE_QUEUE_OBJ) $(AI_WORKER_OBJ) $(VOICE_INPUT_OBJ) $(VOICE_TRANSCRIBER_OBJ) $(OPENAI_TRANSCRIBE_OBJ) $(VOICE_MODE_OBJ) $(SQLITE_QUEUE_OBJ) $(MCP_OBJ) $(TOOL_UTILS_OBJ) $(PROCESS_UTILS_OBJ) $(DUMP_UTILS_OBJ) $(SUBAGENT_MANAGER_OBJ) $(EXPLORE_TOOLS_OBJ) $(BASE64_OBJ) $(HISTORY_FILE_OBJ) $(ARRAY_RESIZE_OBJ) $(HTTP_CLIENT_OBJ) $(SESSION_OBJ) $(SESSION_RESOLVE_OBJ) $(RETRY_LOGIC_OBJ) $(COMPACTION_OBJ) $(BACKGROUND_INIT_OBJ) $(DATA_DIR_OBJ) $(FILE_UTILS_OBJ) $(STRING_UTILS_OBJ) $(TIMESTAMP_UTILS_OBJ) $(FORMAT_UTILS_OBJ) $(ENV_UTILS_OBJ) $(OUTPUT_UTILS_OBJ) $(DIFF_UTILS_OBJ) $(REDACT_UTILS_OBJ) $(TOOL_SLEEP_OBJ) $(TOOL_TODO_OBJ) $(TOOL_IMAGE_OBJ) $(TOOL_SEARCH_OBJ) $(TOOL_FILESYSTEM_OBJ) $(TOOL_BASH_OBJ) $(TOOL_SUBAGENT_OBJ) $(TOOL_REGISTRY_OBJ) $(TOOL_EXECUTOR_OBJ) $(TOOL_DEFINITIONS_OBJ) $(DYNAMIC_TOOLS_OBJ) $(CONTENT_TYPES_OBJ) $(CONVERSATION_STATE_OBJ) $(MESSAGE_BUILDER_OBJ) $(MESSAGE_PARSER_OBJ) $(CONTEXT_SYSTEM_PROMPT_OBJ) $(CONTEXT_ENVIRONMENT_OBJ) $(CONTEXT_KLAWED_MD_OBJ) $(CONTEXT_MEMORY_INJECTION_OBJ) $(UI_OUTPUT_OBJ) $(PRINT_HELPERS_OBJ) $(TOOL_OUTPUT_DISPLAY_OBJ) $(SESSION_TOKEN_USAGE_OBJ) $(SESSION_PERSISTENCE_OBJ) $(API_RESPONSE_OBJ) $(API_BUILDER_OBJ) $(API_CLIENT_OBJ) $(INTERACTIVE_LOOP_OBJ) $(INPUT_HANDLER_OBJ) $(RESPONSE_PROCESSOR_OBJ) $(COMMAND_DISPATCH_OBJ) $(ONESHOT_MODE_OBJ) $(ONESHOT_PROCESSOR_OBJ) $(ONESHOT_OUTPUT_OBJ) $(PROVIDER_COMMAND_OBJ) $(CONFIG_COMMAND_OBJ)
	@mkdir -p $(BUILD_DIR)
...
$(SETTINGS_MENU_OBJ): $(SETTINGS_MENU_SRC) src/settings_menu.h src/config_command.h src/config.h src/tui.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(SETTINGS_MENU_OBJ) $(SETTINGS_MENU_SRC)
...
$(LINE_PRINTER_OBJ): $(LINE_PRINTER_SRC) src/line_printer.h src/tui.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(LINE_PRINTER_OBJ) $(LINE_PRINTER_SRC)

$(MARKDOWN_RENDER_OBJ): $(MARKDOWN_RENDER_SRC) src/tui.h src/markdown_render.h src/klawed_internal.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(MARKDOWN_RENDER_OBJ) $(MARKDOWN_RENDER_SRC)
...
$(VLTRN_BANNER_OBJ): $(VLTRN_BANNER_SRC) src/vltrn_banner.h src/tui.h src/window_manager.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(VLTRN_BANNER_OBJ) $(VLTRN_BANNER_SRC)
...
$(VOICE_TRANSCRIBER_OBJ): $(VOICE_TRANSCRIBER_SRC) src/voice_transcriber.h src/logger.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(VOICE_TRANSCRIBER_OBJ) $(VOICE_TRANSCRIBER_SRC)

$(OPENAI_TRANSCRIBE_OBJ): $(OPENAI_TRANSCRIBE_SRC) src/openai_transcribe.h src/voice_transcriber.h src/logger.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(OPENAI_TRANSCRIBE_OBJ) $(OPENAI_TRANSCRIBE_SRC)

$(VOICE_MODE_OBJ): $(VOICE_MODE_SRC) src/voice_mode.h src/voice_transcriber.h src/tui.h src/tui_input.h src/logger.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(VOICE_MODE_OBJ) $(VOICE_MODE_SRC)
...
$(PROVIDER_OBJ): $(PROVIDER_SRC) src/provider.h src/openai_provider.h src/bedrock_provider.h src/anthropic_provider.h src/deepseek_provider.h src/moonshot_provider.h src/zai_coding_provider.h src/minimax_coding_provider.h src/openai_responses_provider.h src/logger.h
	@mkdir -p $(BUILD_DIR)
...
$(OPENAI_STREAMING_OBJ): $(OPENAI_STREAMING_SRC) src/openai_streaming.h src/provider.h src/logger.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(OPENAI_STREAMING_OBJ) $(OPENAI_STREAMING_SRC)
...
$(ZAI_CODING_PROVIDER_OBJ): $(ZAI_CODING_PROVIDER_SRC) src/zai_coding_provider.h src/provider.h src/logger.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(ZAI_CODING_PROVIDER_OBJ) $(ZAI_CODING_PROVIDER_SRC)

$(MINIMAX_CODING_PROVIDER_OBJ): $(MINIMAX_CODING_PROVIDER_SRC) src/minimax_coding_provider.h src/provider.h src/logger.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(MINIMAX_CODING_PROVIDER_OBJ) $(MINIMAX_CODING_PROVIDER_SRC)

$(KIMI_OAUTH_OBJ): $(KIMI_OAUTH_SRC) src/kimi_oauth.h src/logger.h src/http_client.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(KIMI_OAUTH_OBJ) $(KIMI_OAUTH_SRC)

$(KIMI_CODING_PLAN_PROVIDER_OBJ): $(KIMI_CODING_PLAN_PROVIDER_SRC) src/kimi_coding_plan_provider.h src/kimi_oauth.h src/provider.h src/logger.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(KIMI_CODING_PLAN_PROVIDER_OBJ) $(KIMI_CODING_PLAN_PROVIDER_SRC)

$(OPENAI_OAUTH_OBJ): $(OPENAI_OAUTH_SRC) src/openai_oauth.h src/logger.h src/http_client.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(OPENAI_OAUTH_OBJ) $(OPENAI_OAUTH_SRC)

$(OPENAI_SUB_PROVIDER_OBJ): $(OPENAI_SUB_PROVIDER_SRC) src/openai_sub_provider.h src/openai_oauth.h src/provider.h src/logger.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(OPENAI_SUB_PROVIDER_OBJ) $(OPENAI_SUB_PROVIDER_SRC)

$(ANTHROPIC_OAUTH_OBJ): $(ANTHROPIC_OAUTH_SRC) src/anthropic_oauth.h src/logger.h src/http_client.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(ANTHROPIC_OAUTH_OBJ) $(ANTHROPIC_OAUTH_SRC)

$(ANTHROPIC_SUB_PROVIDER_OBJ): $(ANTHROPIC_SUB_PROVIDER_SRC) src/anthropic_sub_provider.h src/anthropic_oauth.h src/anthropic_provider.h src/provider.h src/logger.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(ANTHROPIC_SUB_PROVIDER_OBJ) $(ANTHROPIC_SUB_PROVIDER_SRC)

$(OPENAI_RESPONSES_PROVIDER_OBJ): $(OPENAI_RESPONSES_PROVIDER_SRC) src/openai_responses_provider.h src/openai_responses.h src/openai_provider.h src/provider.h src/logger.h
	@mkdir -p $(BUILD_DIR)
	$(CC) $(CFLAGS) -c -o $(OPENAI_RESPONSES_PROVIDER_OBJ) $(OPENAI_RESPONSES_PROVIDER_SRC)
...
define BUILD_TOOL_SYSTEM_TEST_OBJS
	...
	@if [ ! -f "$(DYNAMIC_TOOLS_TEST_OBJ)" ]; then \
		echo "Building $(DYNAMIC_TOOLS_TEST_OBJ)..."; \
		$(CC) $(CFLAGS) -DTEST_BUILD -c -o $(DYNAMIC_TOOLS_TEST_OBJ) src/dynamic_tools.c; \
	fi
	@if [ ! -f "$(CODEX_TOOLS_TEST_OBJ)" ]; then \
		echo "Building $(CODEX_TOOLS_TEST_OBJ)..."; \
		$(CC) $(CFLAGS) -DTEST_BUILD -c -o $(CODEX_TOOLS_TEST_OBJ) src/tools/codex_tools.c; \
	fi
endef
...
$(TEST_EDIT_TARGET): $(SRC) $(TEST_EDIT_SRC) $(TEST_COMMON_OBJS)
	...
	@$(CC) -o $(TEST_EDIT_TARGET) $(BUILD_DIR)/claude_test.o $(BUILD_DIR)/test_edit.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_EDIT_REGEX_TARGET): $(SRC) $(TEST_EDIT_REGEX_SRC) $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_EDIT_REGEX_TARGET) $(BUILD_DIR)/claude_test.o $(BUILD_DIR)/test_edit_regex.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_READ_TARGET): $(SRC) $(TEST_READ_SRC) $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_READ_TARGET) $(BUILD_DIR)/claude_read_test.o $(BUILD_DIR)/test_read.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_TODO_WRITE_TARGET): $(SRC) $(TEST_TODO_WRITE_SRC) $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_TODO_WRITE_TARGET) $(BUILD_DIR)/claude_todowrite_test.o $(BUILD_DIR)/test_todo_write.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_BASH_TIMEOUT_TARGET): $(SRC) $(TEST_BASH_TIMEOUT_SRC) $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_BASH_TIMEOUT_TARGET) $(BUILD_DIR)/claude_bash_timeout_test.o $(BUILD_DIR)/test_bash_timeout.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_BASH_STDERR_TARGET): $(SRC) $(TEST_BASH_STDERR_SRC) $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_BASH_STDERR_TARGET) $(BUILD_DIR)/claude_bash_stderr_test.o $(BUILD_DIR)/test_bash_stderr.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_BASH_TRUNCATION_TARGET): $(SRC) $(TEST_BASH_TRUNCATION_SRC) $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_BASH_TRUNCATION_TARGET) $(BUILD_DIR)/claude_bash_truncation_test.o $(BUILD_DIR)/test_bash_truncation.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
# Test target for TUI Streaming Index - tests streaming entry tracking
# Self-contained test with embedded implementations
$(TEST_TUI_STREAMING_INDEX_TARGET): $(TEST_TUI_STREAMING_INDEX_SRC)
	@mkdir -p $(BUILD_DIR)
	@echo "Compiling TUI Streaming Index test suite..."
...
$(TEST_PROVIDER_INIT_FROM_CONFIG_TARGET): $(SRC) tests/test_provider_init_from_config.c $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_PROVIDER_INIT_FROM_CONFIG_TARGET) $(BUILD_DIR)/claude_provider_test.o $(BUILD_DIR)/test_provider_init_from_config.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_PROVIDER_INIT_TARGET): $(SRC) tests/test_provider_init.c $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_PROVIDER_INIT_TARGET) $(BUILD_DIR)/provider_init_test.o $(BUILD_DIR)/test_provider_init.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_MEMORY_DB_TARGET): $(TEST_MEMORY_DB_SRC) $(MEMORY_DB_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(DATA_DIR_OBJ)
	@mkdir -p $(BUILD_DIR)
	@echo "Compiling Memory DB test suite..."
	@$(CC) $(CFLAGS) -o $(TEST_MEMORY_DB_TARGET) $(TEST_MEMORY_DB_SRC) $(MEMORY_DB_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(DATA_DIR_OBJ) $(LDFLAGS)
	@echo ""
	@echo "✓ Memory DB test build successful!"

# Test target for Memory Retraction - tests relation: "retracts" functionality
$(TEST_MEMORY_RETRACT_TARGET): $(TEST_MEMORY_RETRACT_SRC) $(MEMORY_DB_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(DATA_DIR_OBJ)
	@mkdir -p $(BUILD_DIR)
	@echo "Compiling Memory Retraction test suite..."
	@$(CC) $(CFLAGS) -o $(TEST_MEMORY_RETRACT_TARGET) $(TEST_MEMORY_RETRACT_SRC) $(MEMORY_DB_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(DATA_DIR_OBJ) $(LDFLAGS)
	@echo ""
	@echo "✓ Memory Retraction test build successful!"
	@echo ""

# Test target for Token Usage - tests token usage tracking functionality
$(TEST_TOKEN_USAGE_TARGET): $(TEST_TOKEN_USAGE_SRC) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(PERSISTENCE_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(TOKEN_USAGE_DB_OBJ) $(TOKEN_USAGE_DB_MIGRATIONS_OBJ) $(MIGRATIONS_OBJ) $(DATA_DIR_OBJ)
	@mkdir -p $(BUILD_DIR)
	@echo "Compiling Token Usage test suite..."
	@$(CC) $(CFLAGS) -o $(TEST_TOKEN_USAGE_TARGET) $(TEST_TOKEN_USAGE_SRC) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(PERSISTENCE_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(TOKEN_USAGE_DB_OBJ) $(TOKEN_USAGE_DB_MIGRATIONS_OBJ) $(MIGRATIONS_OBJ) $(DATA_DIR_OBJ) $(LDFLAGS)
	@echo ""
...
$(TEST_TOOL_RESULTS_REGRESSION_TARGET): $(SRC) $(TEST_TOOL_RESULTS_REGRESSION_SRC) $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_TOOL_RESULTS_REGRESSION_TARGET) $(BUILD_DIR)/claude_tool_results_test.o $(BUILD_DIR)/test_tool_results_regression.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_MODEL_SWITCH_INTERACTIVE_TARGET): $(SRC) $(TEST_MODEL_SWITCH_INTERACTIVE_SRC) $(TEST_COMMON_OBJS)
	...
	@$(CC) -o $(TEST_MODEL_SWITCH_INTERACTIVE_TARGET) \
		...
		$(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_OPENAI_RESPONSE_PARSING_TARGET): $(SRC) $(TEST_OPENAI_RESPONSE_PARSING_SRC) $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_OPENAI_RESPONSE_PARSING_TARGET) $(BUILD_DIR)/claude_openai_response_test.o $(BUILD_DIR)/test_openai_response_parsing.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_CANCEL_FLOW_TARGET): $(SRC) tests/test_cancel_flow.c $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_CANCEL_FLOW_TARGET) $(BUILD_DIR)/klawed_cancel_flow_test.o $(BUILD_DIR)/test_cancel_flow.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_WRITE_DIFF_INTEGRATION_TARGET): $(SRC) $(TEST_WRITE_DIFF_INTEGRATION_SRC) $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_WRITE_DIFF_INTEGRATION_TARGET) $(BUILD_DIR)/claude_write_diff_test.o $(BUILD_DIR)/test_write_diff_integration.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
	@echo "✓ Write tool diff integration test build successful!"
	@echo ""

# Test target for database rotation
$(TEST_ROTATION_TARGET): $(TEST_ROTATION_SRC) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(PERSISTENCE_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(TOKEN_USAGE_DB_OBJ) $(TOKEN_USAGE_DB_MIGRATIONS_OBJ) $(MIGRATIONS_OBJ) $(DATA_DIR_OBJ)
	@mkdir -p $(BUILD_DIR)
	@echo "Compiling database rotation tests..."
	$(CC) $(CFLAGS) -o $(TEST_ROTATION_TARGET) $(TEST_ROTATION_SRC) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(PERSISTENCE_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(TOKEN_USAGE_DB_OBJ) $(TOKEN_USAGE_DB_MIGRATIONS_OBJ) $(MIGRATIONS_OBJ) $(DATA_DIR_OBJ) $(LDFLAGS)
	@echo ""
...
$(TEST_FUNCTION_CONTEXT_TARGET): $(SRC) $(TEST_FUNCTION_CONTEXT_SRC) $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_FUNCTION_CONTEXT_TARGET) $(BUILD_DIR)/claude_function_context_test.o $(BUILD_DIR)/test_function_context.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
$(TEST_EVENT_LOOP_TARGET): $(SRC) $(TEST_EVENT_LOOP_SRC) $(TEST_STUBS_SRC) $(TEST_COMMON_OBJS)
	...
	@$(CC) $(CFLAGS) -Wno-unused-function -o $(TEST_EVENT_LOOP_TARGET) $(BUILD_DIR)/claude_event_loop_test.o $(TEST_EVENT_LOOP_SRC) $(TEST_STUBS_SRC) $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
help:
	...
	@echo "  make test-memory-db - Build and run Memory DB tests only"
...
# Test target for Command Palette filtering logic (standalone test)
$(TEST_COMMAND_PALETTE_TARGET): $(TEST_COMMAND_PALETTE_SRC)
	@mkdir -p $(BUILD_DIR)
	@echo "Compiling Command Palette test suite..."
	@$(CC) $(CFLAGS) -c -o $(BUILD_DIR)/test_command_palette.o $(TEST_COMMAND_PALETTE_SRC)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_COMMAND_PALETTE_TARGET) $(BUILD_DIR)/test_command_palette.o $(LDFLAGS)
	@echo ""
	@echo "✓ Command Palette test build successful!"
	@echo ""
...
DYNAMIC_TOOLS_TEST_OBJ = $(BUILD_DIR)/dynamic_tools_test.o
CODEX_TOOLS_TEST_OBJ = $(BUILD_DIR)/codex_tools_test.o
...
TEST_COMMON_OBJS = $(LOGGER_OBJ) $(CONFIG_OBJ) $(PERSISTENCE_OBJ) $(MACOS_SQLITE_FIX_OBJ) $(TOKEN_USAGE_DB_OBJ) $(TOKEN_USAGE_DB_MIGRATIONS_OBJ) $(MIGRATIONS_OBJ) $(TODO_OBJ) $(GOAL_OBJ) $(MESSAGE_QUEUE_OBJ) $(OPENAI_MESSAGES_OBJ) $(OPENAI_RESPONSES_OBJ) $(OPENAI_CHAT_PARSER_OBJ) $(BASE64_OBJ) $(PROVIDER_OBJ) $(PROVIDER_CONFIG_LOADER_OBJ) $(OPENAI_PROVIDER_OBJ) $(BEDROCK_PROVIDER_OBJ) $(ANTHROPIC_PROVIDER_OBJ) $(DEEPSEEK_PROVIDER_OBJ) $(MOONSHOT_PROVIDER_OBJ) $(ZAI_CODING_PROVIDER_OBJ) $(MINIMAX_CODING_PROVIDER_OBJ) $(KIMI_OAUTH_OBJ) $(KIMI_CODING_PLAN_PROVIDER_OBJ) $(OPENAI_OAUTH_OBJ) $(OPENAI_SUB_PROVIDER_OBJ) $(OPENAI_RESPONSES_PROVIDER_OBJ) $(ANTHROPIC_OAUTH_OBJ) $(ANTHROPIC_SUB_PROVIDER_OBJ) $(HTTP_CLIENT_OBJ) $(SESSION_OBJ) $(SESSION_RESOLVE_OBJ) $(RETRY_LOGIC_OBJ) $(TOOL_UTILS_OBJ) $(PROCESS_UTILS_OBJ) $(SUBAGENT_MANAGER_OBJ) $(ARRAY_RESIZE_OBJ) $(HISTORY_FILE_OBJ) $(AWS_BEDROCK_OBJ) $(BEDROCK_CONVERSE_OBJ) $(TUI_OBJ) $(TUI_CORE_OBJ) $(TUI_CONVERSATION_OBJ) $(TUI_WINDOW_OBJ) $(TUI_RENDER_OBJ) $(LINE_PRINTER_OBJ) $(MARKDOWN_RENDER_OBJ) $(TUI_MODES_OBJ) $(TUI_INPUT_OBJ) $(TUI_HISTORY_OBJ) $(TUI_COMPLETION_OBJ) $(TUI_PASTE_OBJ) $(TUI_SEARCH_OBJ) $(VLTRN_BANNER_OBJ) $(WINDOW_MANAGER_OBJ) $(COMPLETION_OBJ) $(COMMANDS_OBJ) $(THEME_EXPLORER_OBJ) $(HELP_MODAL_OBJ) $(SETTINGS_MENU_OBJ) $(BUILTIN_THEMES_OBJ) $(AI_WORKER_OBJ) $(VOICE_INPUT_OBJ) $(VOICE_TRANSCRIBER_OBJ) $(OPENAI_TRANSCRIBE_OBJ) $(VOICE_MODE_OBJ) $(MCP_OBJ) $(FILE_SEARCH_OBJ) $(HISTORY_SEARCH_OBJ) $(DUMP_UTILS_OBJ) $(COMPACTION_OBJ) $(BACKGROUND_INIT_OBJ) $(DATA_DIR_OBJ) $(EXPLORE_TOOLS_OBJ) $(CONTENT_TYPES_OBJ) $(CONVERSATION_STATE_OBJ) $(CONVERSATION_PROCESSOR_OBJ) $(MESSAGE_BUILDER_OBJ) $(MESSAGE_PARSER_OBJ) $(FILE_UTILS_OBJ) $(STRING_UTILS_OBJ) $(TIMESTAMP_UTILS_OBJ) $(FORMAT_UTILS_OBJ) $(ENV_UTILS_OBJ) $(OUTPUT_UTILS_OBJ) $(DIFF_UTILS_OBJ) $(REDACT_UTILS_OBJ) $(MODEL_CAPABILITIES_OBJ) $(TOOL_SLEEP_OBJ) $(TOOL_TODO_OBJ) $(TOOL_IMAGE_OBJ) $(TOOL_SEARCH_OBJ) $(TOOL_FILESYSTEM_OBJ) $(TOOL_BASH_OBJ) $(TOOL_SUBAGENT_OBJ) $(CONTEXT_SYSTEM_PROMPT_OBJ) $(CONTEXT_ENVIRONMENT_OBJ) $(CONTEXT_KLAWED_MD_OBJ) $(CONTEXT_MEMORY_INJECTION_OBJ) $(UI_OUTPUT_OBJ) $(PRINT_HELPERS_OBJ) $(TOOL_OUTPUT_DISPLAY_OBJ) $(SESSION_TOKEN_USAGE_OBJ) $(SESSION_PERSISTENCE_OBJ) $(API_RESPONSE_OBJ) $(API_BUILDER_OBJ) $(API_CLIENT_TEST_OBJ) $(INTERACTIVE_LOOP_OBJ) $(INPUT_HANDLER_OBJ) $(RESPONSE_PROCESSOR_OBJ) $(COMMAND_DISPATCH_OBJ) $(ONESHOT_MODE_OBJ) $(ONESHOT_PROCESSOR_OBJ) $(ONESHOT_OUTPUT_OBJ) $(ONESHOT_UI_OBJ) $(PROVIDER_COMMAND_OBJ) $(CONFIG_COMMAND_OBJ) $(MEMORY_DB_OBJ) $(PERPETUAL_LOG_OBJ) $(PERPETUAL_PROMPT_OBJ) $(PERPETUAL_MODE_OBJ)
...
$(TEST_FIREWORKS_TOKEN_FALLBACK_TARGET): $(TEST_FIREWORKS_TOKEN_FALLBACK_SRC)
	@$(CC) $(CFLAGS) -o $(TEST_FIREWORKS_TOKEN_FALLBACK_TARGET) $(TEST_FIREWORKS_TOKEN_FALLBACK_SRC) $(LDFLAGS)

$(TEST_HTTP_CLIENT_TARGET): $(TEST_HTTP_CLIENT_SRC) $(HTTP_CLIENT_OBJ) $(SSE_PARSER_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(RETRY_LOGIC_OBJ) $(DATA_DIR_OBJ)
	@$(CC) $(CFLAGS) -o $(TEST_HTTP_CLIENT_TARGET) $(TEST_HTTP_CLIENT_SRC) $(HTTP_CLIENT_OBJ) $(SSE_PARSER_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(RETRY_LOGIC_OBJ) $(DATA_DIR_OBJ) $(LDFLAGS)
...
# Live streaming test - makes real API calls to test tool call accumulation
$(TEST_STREAMING_LIVE_TARGET): tests/test_streaming_live.c $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(SSE_PARSER_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(DATA_DIR_OBJ)
	@mkdir -p $(BUILD_DIR)
	@echo "Compiling live streaming test..."
	@$(CC) $(CFLAGS) -DARENA_IMPLEMENTATION -o $(TEST_STREAMING_LIVE_TARGET) tests/test_streaming_live.c $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(SSE_PARSER_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(DATA_DIR_OBJ) $(LDFLAGS)
	@echo "✓ Live streaming test built: $(TEST_STREAMING_LIVE_TARGET)"

$(BUILD_DIR)/test_streaming_live2: tests/test_streaming_live2.c $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(SSE_PARSER_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(DATA_DIR_OBJ)
	@mkdir -p $(BUILD_DIR)
	@echo "Compiling live streaming test v2..."
	@$(CC) $(CFLAGS) -DARENA_IMPLEMENTATION -o $@ tests/test_streaming_live2.c $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(SSE_PARSER_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(DATA_DIR_OBJ) $(LDFLAGS)
	@echo "✓ Live streaming test v2 built: $@"

test-streaming-live: check-deps $(TEST_STREAMING_LIVE_TARGET)
	@echo ""
	@echo "Running live streaming test (makes real API calls)..."
	@echo "Usage: KLAWED_ENABLE_STREAMING=1 ./$(TEST_STREAMING_LIVE_TARGET) [provider_name]"
	@echo ""

test-streaming-live2: check-deps $(BUILD_DIR)/test_streaming_live2
	@echo ""
	@echo "Running live streaming test v2 (makes real API calls)..."
	@echo "Usage: KLAWED_ENABLE_STREAMING=1 ./$(BUILD_DIR)/test_streaming_live2 [provider_name]"
	@echo ""

# Moonshot/Kimi streaming test
$(BUILD_DIR)/test_moonshot_streaming: tests/test_moonshot_streaming.c $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(SSE_PARSER_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(DATA_DIR_OBJ)
	@mkdir -p $(BUILD_DIR)
	@echo "Compiling Moonshot streaming test..."
	@$(CC) $(CFLAGS) -DARENA_IMPLEMENTATION -o $@ tests/test_moonshot_streaming.c $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(SSE_PARSER_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(DATA_DIR_OBJ) $(LDFLAGS)
	@echo "✓ Moonshot streaming test built: $@"

test-moonshot-streaming: check-deps $(BUILD_DIR)/test_moonshot_streaming
	@echo ""
...
$(TEST_SQLITE_QUEUE_SEEDING_TARGET): $(SRC) $(TEST_SQLITE_QUEUE_SEEDING_SRC) $(TEST_COMMON_OBJS)
	...
	$(BUILD_TOOL_SYSTEM_TEST_OBJS)
	@echo "Linking test executable..."
	@$(CC) -o $(TEST_SQLITE_QUEUE_SEEDING_TARGET) $(BUILD_DIR)/klawed_sqlite_queue_seeding_test.o $(BUILD_DIR)/test_sqlite_queue_seeding.o $(SQLITE_QUEUE_TEST_OBJ) $(TOOL_REGISTRY_TEST_OBJ) $(TOOL_EXECUTOR_TEST_OBJ) $(TOOL_DEFINITIONS_TEST_OBJ) $(DYNAMIC_TOOLS_TEST_OBJ) $(CODEX_TOOLS_TEST_OBJ) $(CRASH_HANDLER_OBJ) $(SSE_PARSER_OBJ) $(STREAMING_TOOL_ACCUMULATOR_OBJ) $(OPENAI_STREAMING_OBJ) $(TEST_COMMON_OBJS) $(LDFLAGS)
	@echo ""
...
# Test target for session_resolve_partial_id()
$(TEST_SESSION_RESOLVE_TARGET): $(TEST_SESSION_RESOLVE_SRC) $(SESSION_RESOLVE_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(DATA_DIR_OBJ)
	@$(CC) $(CFLAGS) -o $(TEST_SESSION_RESOLVE_TARGET) $(TEST_SESSION_RESOLVE_SRC) $(SESSION_RESOLVE_OBJ) $(LOGGER_OBJ) $(REDACT_UTILS_OBJ) $(DATA_DIR_OBJ) $(LDFLAGS)
