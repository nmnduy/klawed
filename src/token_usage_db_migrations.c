...
static int migration_001_add_metadata(sqlite3 *db) {
    ...
    // Add metadata_id column to existing token_usage table
    const char *alter_sql = "ALTER TABLE token_usage ADD COLUMN metadata_id INTEGER;";
    rc = sqlite3_exec(db, alter_sql, NULL, NULL, &err_msg);
    if (rc != SQLITE_OK) {
        if (strstr(err_msg, "duplicate column name") == NULL) {
            LOG_ERROR("Migration 001 failed (add metadata_id): %s", err_msg);
            sqlite3_free(err_msg);
            return -1;
        }
        sqlite3_free(err_msg);
...
static const TokenUsageMigration MIGRATIONS[] = {
    {
        .version = 1,
        .description = "Add token_usage_metadata table and metadata_id column",
        .up = migration_001_add_metadata
    },
};
