(function() {
    ...
    function formatTitle(name) {
        // Use the real filename (without extension) for display
        return name;
    }
    ...
    function createDemoCard(demo) {
        ...
        card.innerHTML = `
            <div class="aspect-video bg-slate-100 flex items-center justify-center relative group overflow-hidden">
                ${demo.thumbnail ? `<img src="${demo.thumbnail}" alt="Thumbnail for ${formatTitle(demo.name)}" class="h-full w-full object-cover" loading="lazy" />` : `
    ...
    function openVideo(demo) {
        videoModalTitle.textContent = formatTitle(demo.filename.replace(/\.mp4$/i, ''));
