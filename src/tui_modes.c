...
int tui_modes_handle_normal(TUIState *tui, int ch, const char *prompt, void *user_data) {
    ...
    if (tui->normal_mode_last_key == 'g' && ch == 'g') {
        // gg: Go to top
        // scroll_to_top calls prefresh() internally — no redundant refresh needed
    ...
    switch (ch) {
        ...
        case 'G':  // Go to bottom
            // scroll_to_bottom calls prefresh() internally — no redundant refresh needed
        ...
        case 'r':  // Toggle response style
            // Cycle through border → caret → robot → cat → bg → border
            if (tui->response_style == RESPONSE_STYLE_BORDER) {
                ...
                tui->response_style = RESPONSE_STYLE_BG;
                tui_update_status(tui, "Response style: bg (background tint)");
            } else {
