...
int zmq_socket_daemon_mode(ZMQContext *ctx, struct ConversationState *state) {
    ...
    LOG_INFO("ZMQ: =========================================");
    LOG_INFO("ZMQ: Starting ZMQ daemon mode (pure message processor)");
    ...
    printf("ZMQ daemon started on %s (pure message processor mode)\n", ctx->endpoint);
    ...
    // Single zmq_poll item for ZMQ socket only (no stdin)
    ...
    while (ctx->enabled) {
        ...
        if (current_time - last_resend_check >= RESEND_CHECK_INTERVAL_MS) {
            ...
            if (resent > 0) {
                LOG_DEBUG("Resent %d pending message(s)", resent);
            }
            last_resend_check = current_time;
        }

        // Poll ZMQ socket with timeout (100ms)
        int rc = zmq_poll(items, 1, 100);

        if (rc == -1) {
            LOG_ERROR("zmq_poll error: %s", zmq_strerror(errno));
            break;
        }

        if (rc > 0 && (items[0].revents & ZMQ_POLLIN)) {
            char buffer[ZMQ_BUFFER_SIZE];
            int received = zmq_recv(ctx->socket, buffer, sizeof(buffer)-1, ZMQ_DONTWAIT);
            ...
            if (received > 0) {
                ...
                buffer[received] = '\0';
                LOG_INFO("Received message #%d (%d bytes)", message_count, received);
                ...
                if (result != 0) {
                    error_count++;
                    LOG_WARN("Message processing failed (error #%d)", error_count);

                    if (error_count > 10) {
                        LOG_ERROR("Too many consecutive errors (%d), stopping daemon", error_count);
                        printf("ZMQ: Too many consecutive errors (%d), stopping daemon\n", error_count);
                        break;
                    }
                } else {
                    LOG_INFO("Successfully processed message #%d", message_count);
                }
            } else if (received == -1 && errno != EAGAIN) {
                LOG_ERROR("Error receiving from ZMQ: %s", zmq_strerror(errno));
                break;
            }
        }
    }

    LOG_INFO("ZMQ: ZMQ daemon stopped. Processed %d messages.", message_count);
    printf("ZMQ daemon stopped. Processed %d messages.\n", message_count);
    fflush(stdout);
