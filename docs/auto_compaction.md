...
Compaction is performed **at the end of an AI turn** when waiting for user input, provided the threshold has been reached. This means:
- Context can go slightly over the threshold during tool execution chains
- Compaction only runs when the AI has finished responding (no more tool calls)
