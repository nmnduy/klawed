...
Multiple klawed agents (main process + subagents) can share a single browser simultaneously. Each agent gets its own isolated browser tab:

```
┌──────────────────┐   ┌──────────────────┐   ┌──────────────────┐
│  klawed (main)   │   │  klawed subagent │   │ explore subagent │
│  PID: 1234       │   │  PID: 5678       │   │  PID: 9012       │
│  agentId: a1b2.. │   │  agentId: c3d4.. │   │  agentId: e5f6.. │
└───────┬──────────┘   └───────┬──────────┘   └───────┬──────────┘
        │                      │                      │
        ▼                      ▼                      ▼
┌───────────────┐     ┌───────────────┐     ┌───────────────┐
│ Tab: "Klawed  │     │ Tab: "Klawed  │     │ Tab: "Klawed  │
...
1. **Automatic isolation**: Each agent process is identified by a hash of its PID + hostname. The Go host automatically creates a dedicated browser tab for each unique agent on its first BrowserControl call.

2. **Transparent routing**: All navigation, DOM interaction, evaluate, and screenshot commands are automatically routed to the agent's assigned tab. No tab management needed.

3. **Shared commands**: `listTabs`, `getActiveTab`, `newTab`, `closeTab`, `switchTab`, `ping`, `getInfo` work globally and can see/manage all tabs.

4. **Session lifecycle**: Agent tabs are automatically cleaned up after 10 minutes of inactivity. If a tab is externally closed, the next command from that agent auto-creates a new one.

### Manual control

```bash
# Force a new session (fresh tab)
browser_ctl --new-session navigate https://example.com

# Use a custom agent ID
browser_ctl --agent-id my-custom-agent navigate https://example.com

# The agent ID is shown in responses
browser_ctl navigate https://example.com
## Agent tab: 42
```
