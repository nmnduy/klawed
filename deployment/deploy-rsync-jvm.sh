...
echo "================================"
echo "FileSurf v2 Remote Deployment (JVM)"
...
# Step 2: Build JVM application
echo "Step 2: Building JVM application..."
...
# Step 4: Run locally to verify JVM build works
echo "Step 4: Running JVM build locally to verify..."
...
# Start the JVM application in background
...
if ! kill -0 "$LOCAL_PID" 2>/dev/null; then
    echo "   ✗ ERROR: JVM application failed to start!"
...
if kill -0 "$LOCAL_PID" 2>/dev/null; then
    echo "   ✓ JVM application ran successfully for $LOCAL_TEST_DURATION seconds"
    echo "   Stopping local test instance..."
    kill "$LOCAL_PID" 2>/dev/null || true
    wait "$LOCAL_PID" 2>/dev/null || true
else
    echo "   ✗ ERROR: JVM application crashed during test run!"
    rm -rf "$LOCAL_TEST_DIR"
    exit 1
fi
...
echo "================================"
echo ""
echo "The JVM application has been built locally and deployed to $REMOTE_HOST"
echo ""
