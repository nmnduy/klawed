...
**CRITICAL PRINCIPLE**: Keep Qute templates as **pure HTML structure**. Move all logic to JavaScript files.

#### Why This Matters

Qute templates automatically HTML-escape all output to prevent XSS attacks. This is **good for security** but **breaks JSON, JavaScript code, and structured data** when embedded in templates.

**The Problem**:
```html
<!-- ❌ BAD: Qute escapes the JSON -->
<script type="application/ld+json">
{
  "title": "{post.title}"  <!-- Becomes: "title": "&quot;Test&quot;" -->
}
</script>
```
Result: **Invalid JSON that breaks search engines and parsers!**

#### The Solution: Client-Side Logic

**✅ GOOD**: Pass data as JSON, generate logic in JavaScript

1. **Server (Java)**: Serialize data with Jackson
   ```java
   Map<String, Object> data = new HashMap<>();
   data.put("title", post.getTitle());
   String json = OBJECT_MAPPER.writeValueAsString(data);  // Safe!
   return template.data("jsonData", json);
   ```

2. **Template (HTML)**: Just pass the data
   ```html
   <script>
     window.__PAGE_DATA__ = {jsonData.raw};  // .raw prevents double-escaping
   </script>
   <script defer src="{inject:jsHelper.path('myFeature')}"></script>
   ```

3. **JavaScript (Logic)**: Generate and inject dynamic content
   ```javascript
   const data = window.__PAGE_DATA__;
   // Generate JSON-LD, build UI, handle events, etc.
   ```

#### Template Guidelines

**✅ DO**:
- Use templates for **HTML structure only**
- Pass data as **JSON objects** (serialized with Jackson)
- Use `{variable.raw}` for **already-safe** JSON strings
- Load JavaScript files with `defer` or `type="module"`
- Keep templates **simple and declarative**

**❌ DON'T**:
- Don't embed JSON literals in templates with Qute variables
- Don't build JavaScript code inline with dynamic values
- Don't try to manually escape strings in templates
- Don't use `{#if}` for complex business logic
- Don't mix server-side templating with client-side code generation

#### Examples

**❌ BAD - Inline JSON/JavaScript**:
```html
<script>
  const config = {
    title: "{post.title}",           // ❌ Escaping issues!
    count: {post.viewCount},         // ❌ Type issues!
    items: [{#for item in items}"{item}",{/for}]  // ❌ Syntax errors!
  };
</script>
```

**✅ GOOD - Pass JSON, Logic in JS**:
```html
<!-- Template: Just pass the data -->
<script>
  window.__PAGE_CONFIG__ = {configJson.raw};
</script>
<script defer src="{inject:jsHelper.path('pageHandler')}"></script>
```

```javascript
// pageHandler.js: Contains all the logic
function initPage() {
  const config = window.__PAGE_CONFIG__;
  const items = config.items.map(item => processItem(item));
  renderUI(items);
}
...
See the blog post structured data refactoring:
- **Before**: Server generated JSON-LD string → Template escaped it → Broken JSON ❌
- **After**: Server serialized to JSON → Template passed it → JS generated JSON-LD ✅

Full details: `docs/CLIENT_SIDE_STRUCTURED_DATA.md`

#### Benefits

1. **No Escaping Issues**: JSON and JavaScript handled by proper parsers
2. **Better Testing**: JavaScript logic can be unit tested independently
3. **Cleaner Code**: Clear separation between structure (HTML) and behavior (JS)
4. **Safer**: Jackson serialization prevents XSS automatically
5. **Maintainable**: Changes to logic don't require template changes

#### When to Use Qute Features

Qute's `{#if}`, `{#for}`, and `{#include}` are **perfect for**:
- ✅ Conditional HTML rendering: `{#if user.isAdmin}<div>Admin Panel</div>{/if}`
- ✅ Iterating over HTML elements: `{#for post in posts}<article>...</article>{/for}`
- ✅ Template composition: `{#include header.html /}`
- ✅ Simple text output: `<h1>{post.title}</h1>`

Qute is **NOT meant for**:
- ❌ Building JSON objects
- ❌ Generating JavaScript code
- ❌ Complex business logic
- ❌ Data transformations (do in Java instead)
