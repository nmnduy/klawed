...
- **Tools**: 
  ...
  - `KLAWED_SHORT_SCREEN_THRESHOLD` - Screen height threshold (in lines) below which the TODO banner is hidden to save vertical space (default: 15, 0=always hide TODO banner)
...
- **Nerd Font Icons**: `KLAWED_NO_NERD_FONT=1` to disable Nerd Font icons and fall back to Unicode text/emoji. By default, the TUI uses Nerd Font glyphs for role tags (`` assistant, `` user, `` reasoning, `` tools, ``/``/`` TODO states) and the terminal title folder icon (``)
